...
main :: proc() {
	...
	wndclass.lpfnWndProc = proc "std" (hwnd: w.HWND, message: u32, wParam: uintptr, lParam: int) -> int {
		...
		switch message {
			...
			case w.WM_CREATE:
				...
				round_mode := i32(w.DWM_WINDOW_CORNER_PREFERENCE.DONOTROUND)
	...
	main_loop: for {
		...
		delta := f32(clock_current - clock_previous) / f32(clock_previous)
		...
		game.update_and_render(&game_renderer, &game_input)
