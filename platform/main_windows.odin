...
main :: proc() {
	...
	wndclass.lpfnWndProc = proc "std" (hwnd: w.HWND, message: u32, wParam: uintptr, lParam: int) -> int {
		...
		switch message {
			...
			case w.WM_CREATE:
				...
				w.DwmSetWindowAttribute(hwnd, cast(u32) w.DWMWINDOWATTRIBUTE.DWMWA_WINDOW_CORNER_PREFERENCE, &round_mode, size_of(type_of(round_mode)))
	...
	w.CreateWindowExW(0, wndclass.lpszClassName, raw_data([]u16{'O', 'p', 'e', 'n', ' ', 'R', 'o', 'y', 'a', 'l', 'e', 0}),
	...
	main_loop: for {
		...
		game_renderer.procs = platform_renderer.procs
		game.update_and_render(&game_input, &game_renderer)
